        ...
        <div class="connexion-window-admin">
            <header class="profil-header"
                ...
                <span class="material-icons"
                    style="margin-bottom: 30px; margin-right: 20px; user-select: none; font-size: 40px; color: #003366;">pallet</span>
            </header>
                        ...
                        <li style="user-select: none; display: flex; align-items: center; padding: 15px;">
                            <div class="text" style="user-select: none; flex-grow: 1;">
                                ...
                                <div style="font-size: 0.85rem; color: #555; margin-top: 4px;">
                                    <em><?= esc($commande->email_client) ?></em>
                                </div>
                            </div>
                            <div style="user-select: none; display: flex; gap: 15px; align-items: center;">
                                <?php if ($commande->statut != 'En attente du paiement'): ?>
                                    <a href="<?= site_url('admin/change-statut/' . $commande->id . '/prev') ?>"
                                        style="color: #003366; text-decoration: none; rotate: 180deg; margin-bottom: 7px;"
                                        title="Statut précédent">
                                        <span class="material-icons">play_circle_filled</span>
                                    </a>
                                <?php endif; ?>

                                <span class="status-badge status-<?= url_title(strtolower($commande->statut)) ?>"
                                    style="font-weight: bold; padding: 5px 12px; border-radius: 20px; background: #793131ff; min-width: 140px; text-align: center;">
                                    <?= esc($commande->statut) ?>
                                </span>

                                <?php if ($commande->statut != 'Livrée'): ?>
                                    <a href="<?= site_url('admin/change-statut/' . $commande->id . '/next') ?>"
                                        style="color: #003366; text-decoration: none; margin-top: 2px;" title="Statut suivant">
                                        <span class="material-icons">play_circle_filled</span>
                                    </a>
                                <?php endif; ?>
                            </div>
