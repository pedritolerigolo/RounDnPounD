...
<head>
    ...
    <title>RounDnPounD | Modif Ingredient</title>
...
<body>
    ...
    <main class="profil-page">
        <div class="connexion-window-admin">
            <h1>Modifier le Ingredient</h1>

            <form action="<?= site_url('admin/update-ingredient/' . $ingredient['id']) ?>" method="post"
                enctype="multipart/form-data">
                <?= csrf_field() ?>

                <label>Nom de l'ingrédient</label>
                <input type="text" name="nom" value="<?= esc($ingredient['nom']) ?>" required>

                <label>Quantité</label>
                <input type="number" step="0.01" name="quantite" value="<?= esc($ingredient['quantite']) ?>" required>
                ...
                <select name="fournisseur" required
                    ...
                    <?php foreach ($allFournisseurs as $f): ?>
                        <?php
                        $fId = is_array($f) ? $f['id'] : $f->id;
                        $fNom = is_array($f) ? $f['nom'] : $f->nom;
                        $isSelected = ($fId == $ingredient['fournisseur']) ? 'selected' : '';
                        ?>
                        <option value="<?= $fId ?>" <?= $isSelected ?>>
                            <?= esc($fNom) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
