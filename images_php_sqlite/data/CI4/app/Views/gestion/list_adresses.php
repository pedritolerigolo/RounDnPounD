        ...
        <div class="connexion-window-admin">
            <header class="profil-header"
                ...
                <span class="material-icons"
                    style="margin-bottom: 30px; margin-right: 20px; user-select: none; font-size: 40px; color: #003366;">location_on</span>
            </header>

            <div class="profil-section">
                <ul class="action-list">
                    <?php if (empty($adresses)): ?>
                        <li style="user-select: none; padding: 20px; text-align: center; color: #888;">Aucune adresse d'enregistrée</li>
                    <?php endif; ?>
                    ...
                    <?php foreach ($adresses as $adr): ?>
                        <li style="user-select: none; display: flex; align-items: center; padding: 15px;">
                            <div class="text" style="user-select: none; flex-grow: 1;">
                                <strong><?= esc($adr->addresse_complete . ', ' . $adr->code_postal . ' ' . $adr->ville . ', ' . $adr->pays) ?></strong>
                            </div>
                            <div style="user-select: none; display: flex; gap: 15px; align-items: center;">

                                <a href="<?= site_url('delete-adresse/' . $adr->id) ?>" title="Supprimer"
                                    onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette addesse ? Cette action est irréversible.')"
            ...
            <div style="user-select: none; margin-top: 30px; text-align: center;">
                <a href="<?= site_url('adresse/ajouter') ?>"
